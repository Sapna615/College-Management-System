...
const TeacherTimetable = () => {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      console.log('Current token available:', !!localStorage.getItem('token'))
      ...
      if (!token || token.length < 100) {
        ...
        alert('Please log in again to access your timetable.')
        window.location.href = '/login'
        return
      }
      
      // Fetch timetables first (required)
      const timetablesData = await timetableAPI.getTeacherTimetable()
      console.log('Teacher timetables received:', timetablesData?.length || 0)
      
      // If no timetables received, it might be a token issue
      if (!timetablesData || timetablesData.length === 0) {
        console.warn('No timetables received - checking if token is valid')
      ...
      // Fetch courses (required)
      const coursesData = await courseAPI.getAllCourses()
      console.log('Courses received:', coursesData?.length || 0)
      
      // Try to fetch teacher profile (optional - don't fail if this fails)
      let teacherData = null
      try {
        teacherData = await teacherAPI.getCurrentTeacherProfile()
        console.log('Teacher Profile received:', !!teacherData)
      } catch (profileError) {
        console.warn('Teacher profile fetch failed, continuing without it:', profileError.message)
        // Continue without teacher profile - timetables are already filtered by backend
      }
      ...
      console.log('Final data set:', {
        timetablesCount: timetablesData?.length || 0,
        coursesCount: coursesData?.length || 0,
        hasTeacherProfile: !!teacherData
      })
    } catch (error) {
  ...
  const filteredTimetables = timetables.filter(timetable => {
    // No need to filter by teacher since backend already returns only this teacher's timetables
        ...
        <div>
          ...
          {!teacherProfile && (
            <p className="text-sm text-orange-600 font-medium">
              Your teaching schedule (Profile not available)
            </p>
          )}
        </div>
        {selectedCourse && (
              ...
              {filteredTimetables.length > 0 ? (
                filteredTimetables.map((timetable, index) => (
                  <tr key={index} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">
                        {teacherProfile?.user?.name || timetable.teacherName || 'You'}
                      </div>
                      <div className="text-xs text-gray-500">
                        {teacherProfile?.employeeId || 'Current Teacher'}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td colSpan="6" className="px-6 py-12 text-center">
                    ...
                    <p className="text-sm text-gray-400">
                      {teacherProfile 
                        ...
                        : 'No classes found in your schedule. Contact admin if you expect to see classes.'
                      }
                    </p>
                  </td>
                </tr>
